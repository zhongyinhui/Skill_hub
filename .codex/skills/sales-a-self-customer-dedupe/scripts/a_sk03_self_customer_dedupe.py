...
SKILL_ID = "A-SK03"
CONFIRMED = {True, "true", "confirmed", "approved", "yes", "已确认", "确认", "通过", "无冲突", "新建确认"}
PROHIBITED = {"P(win)", "amount_factor", "urgency_factor", "relationship_factor", "rating_score", "HI", "customer_rating", "current_stage"}
...
def key_for(data, customer_id):
    if data.get("idempotency_key"):
        return str(data["idempotency_key"])
    seed = f"{data.get('customer_name','')}|{data.get('sales_id','')}|{customer_id}"
...
def normalize_index_source_type(value):
    allowed = {"市场线索", "客服转接", "客户成功", "转介绍", "直播", "投流", "销售自开发", "其他"}
    if value in allowed:
        return value
    value_text = str(value or "").strip().lower()
    if any(token in value_text for token in ("sales", "self", "销售", "微信", "wxrec")):
        return "销售自开发"
    if any(token in value_text for token in ("market", "市场", "投流", "直播")):
        return "市场线索"
    if any(token in value_text for token in ("customer_success", "客户成功")):
        return "客户成功"
    if any(token in value_text for token in ("客服", "service")):
        return "客服转接"
    return "销售自开发"


def normalize_index_department(value):
    allowed = {"市场", "客服", "客户成功", "销售", "总经办", "外部"}
    if value in allowed:
        return value
    value_text = str(value or "").strip().lower()
    if any(token in value_text for token in ("market", "市场", "投流", "直播")):
    ...
    if any(token in value_text for token in ("客服", "service")):
        return "客服"
    if any(token in value_text for token in ("外部", "external")):
        return "外部"
    return "销售"


def source_index_fields(data, customer_id, customer_name, sales_id, created_at):
    source_id = data.get("source_id") or data.get("source_record_id")
    if not source_id:
        return {}
    candidates = as_list(data.get("duplicate_candidate_ids"))
    return clean({
        "source_map_id": data.get("source_map_id") or data.get("index_id") or f"SMAP-{hash12(source_id)}",
        "source_id": source_id,
        "source_type": normalize_index_source_type(data.get("source_type")),
        "source_department": normalize_index_department(data.get("source_department")),
        "raw_customer_name": data.get("raw_customer_name") or customer_name,
        "raw_phone": data.get("raw_phone") or data.get("phone"),
        "candidate_customer_ids": candidates or None,
...
def plan(data, config):
    ...
    duplicates = as_list(data.get("duplicate_candidate_ids"))
    if duplicates and not confirmed(data.get("duplicate_confirm_status")):
        return "needs_confirm", "", {}, [], ["duplicate candidates exist; confirm whether to bind or create"], {}
    if not confirmed(data.get("create_confirmed")):
        return "needs_confirm", "", {}, [], ["new no-source customer creation is not human-confirmed"], {}

    customer_id = data.get("customer_id") or f"CUST-{hashlib.sha256((customer_name or '').encode('utf-8')).hexdigest()[:12].upper()}"
    key = key_for(data, customer_id)
    if key in set(config.get("existing_idempotency_keys", [])):
        blocked.append("duplicate idempotency_key; refusing duplicate creation")

    created_at = now_iso()
    aliases = as_list(data.get("alias_names")) or [customer_name]
    writes = [
        {"table": "A03_all_customer_files", "operation": "create", "idempotency_key": key, "fields": clean({
            ...
            "individual_customer_table_url": data.get("individual_customer_table_url"),
            "ledger_table_ref": data.get("ledger_table_ref"),
            "evidence_ref_table_ref": data.get("evidence_ref_table_ref"),
            "artifacts_ref_table_ref": data.get("artifacts_ref_table_ref"),
            "handoff_ref_table_ref": data.get("handoff_ref_table_ref"),
            "latest_snapshot_table_ref": data.get("latest_snapshot_table_ref"),
            "created_at": created_at,
            "updated_at": created_at,
        })},
    ]
    index_fields = source_index_fields(data, customer_id, customer_name, sales_id, created_at)
    if index_fields:
        writes.append({"table": "A06_a_line_index", "operation": "upsert", "idempotency_key": f"{key}-INDEX-{hash12(index_fields['source_id'])}", "fields": index_fields})
    for idx, alias in enumerate(aliases):
        writes.append({"table": "A01_customer_alias", "operation": "create", "idempotency_key": f"{key}-ALIAS-{idx+1}", "fields": clean({
            "alias_id": f"ALIAS-{hashlib.sha256((customer_id + str(alias)).encode('utf-8')).hexdigest()[:12]}",
            "customer_id": customer_id,
            "alias_name": alias,
            "alias_type": "self_reported",
            "source_line": SKILL_ID,
            "source_record_id": data.get("source_record_id"),
            "confidence_score": data.get("confidence_score", 1),
            "confirm_status": data.get("create_confirmed"),
            "created_by": sales_id,
            "created_at": created_at,
            "status": "active",
            "remark": data.get("remark"),
        })})
    blocked.extend(validate_writes(config, writes))
    status = "rejected" if blocked else "pass"
    ref_fields = config.get("customer_table_model", {}).get("per_customer_ref_fields", [])
    missing_refs = [field for field in ref_fields if not data.get(field)]
    return status, key, {"customer_id": customer_id, "customer_name": customer_name, "sales_id": sales_id}, writes, blocked, {
        "customer_id": customer_id,
        "individual_customer_container_ready": not missing_refs,
        "missing_customer_container_refs": missing_refs,
        "auto_customer_container_planned": should_auto_create_container(data, config, {"missing_customer_container_refs": missing_refs}),
        ...
        } if should_auto_create_container(data, config, {"missing_customer_container_refs": missing_refs}) else {},
    }


def main():
    parser = argparse.ArgumentParser(description="A-SK03 no-source customer dedupe/create validator and executor")
