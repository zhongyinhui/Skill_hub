...
说“进入正式分支 / 合到 master / 团队都能用” -> 必须创建 PR 到 master。
...
| 直接 push 到 `master` / `release` | 正式分支 | 不允许普通同事默认执行 | 应改走 PR |
...
如果同事需要 HTML 演示页、可视化操作手册、页面原型或浏览器可打开的说明文件，Codex 默认使用单个虚拟页面文件：
...
- CSS 和 JavaScript 默认写在同一个 `index.html` 里。
- 多个页面效果用标签页、折叠区、hash route 或前端状态模拟。
- 不默认创建 `pages/*.html`、`assets/` 或多个真实 HTML 文件。
- 不把“多文件真实子页面 vs 单文件虚拟页面”作为默认问题询问用户。
- 只有用户明确要求多文件，或存在必须拆分的技术原因时，才改为多文件结构，并先说明原因。

这条规则是为了方便非技术同事保存、预览、上传、回滚和交接。
