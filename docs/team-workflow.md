...
这里的 `.codex/skills/...` 指当前仓库里的项目级目录，例如 `D:\中隐会\Skills_hub\.codex\skills\...`，不是 C 盘全局 Codex 目录。全局目录通常是 `C:\Users\<user>\.codex\skills\...`，只有明确要跨项目使用时才考虑。项目入口在选择器中应显示当前项目来源，例如 `Skills_hub`；同步后不要承诺固定秒数自动加载，以刷新 skill 列表或新开线程后的可见结果为准。
