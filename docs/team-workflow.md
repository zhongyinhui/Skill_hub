...
管理者明确说“我确认直接更新 master” -> 可以在校验通过并说明影响后直接 push。
...
| 从个人分支进入 `master` | `master` | 普通同事必须；管理者可选择 | 影响团队稳定基线 |
| 从个人分支进入 `release` | `release` | 必须 | 影响打包发布和 Release |
| 直接 push 到 `master` / `release` | 正式分支 | 只允许管理者明确确认后执行 | 会绕过 PR 过程记录 |
...
如果仓库管理者自己就是最终负责人，PR 不是为了“自己审批自己”，而是用于变更说明和决策留档。

推荐顺序：

1. 先 push 到个人工作分支，保留个人版本。
2. 需要进入 `master` 时，优先创建 PR 作为留档入口。
3. PR 作者不能 approve 自己；管理者可在检查 diff、校验结果和影响范围后直接 merge，并在 PR 描述或合并说明中写明“管理者确认合并”。
4. 如果管理者明确要求不走 PR，Agent 才可以直接 push 到 `master`；执行前必须说明这会绕过 PR 留档，并再次确认不会带入测试内容或无关提交。
...
- 强确认动作：merge、tag、release、删除或废弃 skill、直接更新 `master` / `release`、修改 workflow 或分支保护相关规则。
