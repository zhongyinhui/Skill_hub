...
- 管理者自己推进 `master` 时，是否区分 PR 留档后 merge 和直接更新 `master` 两种方式。
...
- 普通同事说“进入正式分支”“合到 master”“团队正式使用”时，创建 PR 到 `master`。
- 仓库管理者进入 `master` 时，推荐创建 PR 留档后由管理者确认 merge；如果管理者明确要求直接更新 `master`，先说明会绕过 PR 留档，再在校验通过后执行。
