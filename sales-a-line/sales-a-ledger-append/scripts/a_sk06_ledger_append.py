...
PROHIBITED = {"customer_rating", "current_stage", "P(win)", "rating_score", "HI"}
...
def transform_writes(config, writes):
    ...
    for write in writes:
        table = write["table"]
        attachment_mode = config.get("attachment_tables", {}).get(table, {}).get("mode")
        if attachment_mode == "ledger_json":
            ...
            new_write["attachments"] = [{
                "field": config.get("attachment_tables", {}).get(table, {}).get("ledger_field", "ledger"),
...
def upsert_lookup_conditions(write):
...
def plan(data, config):
    ...
    blocked = []
    required = ["customer_id", "sales_id", "effective_events_summary", "confirm_status", "evidence_ids"]
    for field in required:
        if not package.get(field):
            blocked.append(f"missing required input: {field}")
    if not package.get("validated_by_a_sk05"):
        blocked.append("package is not marked validated_by_a_sk05")
    if not as_list(package.get("evidence_ids")):
        blocked.append("missing evidence_ids; formal ledger append is blocked")
    if not confirmed(package.get("confirm_status")):
        blocked.append("confirm_status is not confirmed")

    created_at = now_iso()
    record_time = ledger_record_time(package)
    source_sessions = as_list(package.get("source_session_ids"))
    seed = f"{package.get('customer_id','')}|{package.get('work_date','')}|{json.dumps(source_sessions, ensure_ascii=False)}"
    key = package.get("idempotency_key") or f"{SKILL_ID}-{hashlib.sha256(seed.encode('utf-8')).hexdigest()[:16]}"
    if key in set(config.get("existing_idempotency_keys", [])):
        blocked.append("duplicate idempotency_key; refusing duplicate ledger append")

    target_input = dict(data)
    if isinstance(package, dict):
        target_input.update(package)
    target = individual_container_target(target_input, config)
    ...
    write = {"table": "A99_individual_customer_container", "operation": "create", "idempotency_key": key, "base_token": target.get("base_token"), "table_id": target.get("table_id"), "target_ref": target.get("source_ref"), "fields": clean({
        "ledger_block_id": package.get("ledger_block_id", f"LEDGER-{key[-16:]}"),
        "customer_id": package.get("customer_id"),
        "ledger_record_time": record_time,
        "block_type": package.get("block_type", "b_line_validated_fact"),
        "event_date": package.get("work_date", created_at),
        "sales_id": package.get("sales_id"),
        "source_line": SKILL_ID,
        "source_session_ids": source_sessions,
        "event_summary": package.get("effective_events_summary"),
        "stage_before": package.get("stage_before"),
        "stage_after": package.get("stage_after"),
        "need_delta": package.get("need_delta"),
        "objection_delta": package.get("objection_delta"),
        "buying_signal_delta": package.get("buying_signal_delta"),
        "risk_flags": package.get("risk_flags"),
        "evidence_refs": as_list(package.get("evidence_ids")),
        "artifact_refs": as_list(package.get("artifact_ids")),
        "snapshot_update_required": package.get("snapshot_update_required", True),
        "confirm_status": package.get("confirm_status"),
        "created_at": created_at,
    })}
    blocked.extend(validate(config, [write]))
    has_hard_block = bool(blocked)
    missing_target = not (target.get("base_token") and target.get("table_id"))
    if missing_target:
        blocked.append("missing per-customer ledger target: provide individual_customer_base_token + individual_customer_table_id, or a resolvable per-customer base URL")
    status = "rejected" if has_hard_block else "needs_confirm" if missing_target else "pass"
    return status, key, package, [write], blocked, {
        "ledger_block_id": write["fields"].get("ledger_block_id"),
        "ledger_record_time": record_time,
...
def main():
    parser = argparse.ArgumentParser(description="A-SK06 customer ledger append validator and executor")
    ...
    try:
        ...
        if args.execute and status == "pass":
            ...
            if direct_results is not None:
                ...
                status = "error"
                blocked.append("missing lark-cli command template for ledger append")
            else:
